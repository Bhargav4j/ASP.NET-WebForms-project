...
for /f "delims=" %%i in ('powershell -Command "'!IMAGE_NAME!' -replace '[^a-zA-Z0-9]', '-' -replace '^-+' -replace '-+$'"') do set IMAGE_NAME=%%i
...
for /f "delims=" %%i in ('powershell -Command "'!IMAGE_TAG!' -replace '[^a-zA-Z0-9.-]', '-' -replace '^-+' -replace '-+$'"') do set IMAGE_TAG=%%i
...
echo.
echo Next steps:
echo 1. Update your deployment manifests with this image URI
